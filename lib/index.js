 ...
 * @param {Boolean} [options.showSuggestionsOnlyWhileTyping=false] If `false`, indicates while typing the forwardGeocode and getSuggestions API will be called. If `true`, indicates that while typing only the getSuggestions API will be called and only suggestion results rendered.
...
MaplibreGeocoder.prototype = {
  options: {
    ...
    showSuggestionsOnlyWhileTyping: false,
    minLength: 2,
  ...
  _onKeyDown: function (e) {
    ...
    if (
      ...
      this.options.showResultsWhileTyping
      // (this.options.showResultsWhileTyping ||
      // this.options.showSuggestionsOnlyWhileTyping)
    ) {
      this._handleInputChange(target.value);
    }
  },

  _handleInputChange: function (input) {
    // only show suggestions when typing
    if (
      this.geocoderApi.getSuggestions &&
      this.options.showSuggestionsOnlyWhileTyping
    ) {
      var suggestions = this._getSuggestions(input) || [];
      suggestions
        .then(
          function (response) {
      ...
      // Perform search and suggestions (if applicable) while typing
  ...
  setInput: function (searchInput) {
    ...
    if (
      ...
      this._handleInputChange(searchInput);
    }
    return this;
  },
